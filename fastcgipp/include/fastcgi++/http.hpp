...
namespace Fastcgipp
{
	...
	namespace Http
	{
		...
		{
			typedef std::map<std::basic_string<charT>, std::basic_string<charT> > NameValuePair;
			typedef NameValuePair Cookies;
			typedef NameValuePair Gets;
			 ...
			 */
			virtual bool fillPostBuffer(const char* data, size_t size);

			//! Parses "multipart/form-data" http post data into the posts object
			virtual void parsePostsMultipart();

			//! Parses "application/x-www-form-urlencoded" post data into the posts object.
			virtual void parsePostsUrlEncoded();

			//! Clear the post buffer
			virtual void clearPostBuffer() { postBuffer.reset(); pPostBuffer=0; }

			Environment(): requestMethod(HTTP_METHOD_ERROR), etag(0), keepAlive(0), contentLength(0), serverPort(0), remotePort(0), boundarySize(0), pPostBuffer(NULL) {}
