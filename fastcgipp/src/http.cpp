...
class UnsupportedCodeException : public ::std::exception
{
	protected:
		const char* msg;

	public:
		inline UnsupportedCodeException(const char* msg) : msg(msg) {};
		virtual inline ~UnsupportedCodeException() throw() {};
...
{
	throw UnsupportedCodeException("Conversion to wstring is not supported");
}
...
template<class charT> void Fastcgipp::Http::Environment<charT>::fill(const char* data, size_t size)
{
	...
	{{
		...
		std::basic_string<charT> nameStr;
		std::basic_string<charT> valueStr;
		charToString(name, nameSize, nameStr);
		charToString(name, nameSize, valueStr);

		this->params[nameStr] = valueStr;
